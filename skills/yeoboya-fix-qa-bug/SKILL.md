...
- `stages.write-qa.status` ∈ {`done`, `published`}
- `stages.fix-qa-bug.status` 임의 (반복 가능)
...
`progress.stages.fix-qa-bug.status = "done"` 업데이트. (재호출 시 다시 todo로 재설정 후 done 전이)

```
QA 버그 수정 완료. 더 발견된 사항이 있으면 /yeoboya-continue-work에서 다시 QA 버그 수정을 선택하세요.
없으면 작업 종결로 진행 가능.
