...
**작성 문체 (모든 문장에 적용)**:
- 개조식·간결체. 구어체·군더더기 배제, 핵심 키워드를 문장 앞에. 한 문장 = 한 정보.
- **흐름 요약은 1문장**으로 짧게 — 이 페르소나의 핵심 동작·분기만. 여러 문장으로 늘어놓지 않는다.
- **플랫폼 용어 통일** — 채널은 native / web / webview로, 플랫폼은 `workspace.platform` 값(iOS 또는 Android) 하나로 표기. "모바일 / 앱" 등 모호·혼용 표현 금지.
...
5. **§1 페르소나별 흐름 작성** — 페르소나마다 다음 3요소를 순서대로 작성한다:
   a. **흐름 요약** — 이 페르소나가 무엇을 하는지 **1문장**으로 간결히(핵심 동작·분기·트리거만; 길게 늘어놓지 않는다).
...
- [ ] 흐름 요약이 1문장·간결(구어체·군더더기 없음), 플랫폼 표기가 iOS/Android 하나로 통일(모바일/앱 등 모호어 없음)
