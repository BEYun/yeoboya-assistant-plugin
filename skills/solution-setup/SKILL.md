...
1. **서비스 선택** — 먼저 이것만 묻는다. 보기: 1. 달라 2. 클럽라이브 3. 여보야 4. 클럽5678 5. 하루온. 답을 받기 전에는 다음 질문을 하지 않는다.
...
1. `__notion-fetch`로 팀원 목록 DB(위 `collection://…` URL 또는 DB 페이지)를 열어 자식 행 목록을 받는다. 반환된 행 중 `이름`이 `<작업자>`와 일치하는 행을 찾아 그 행 페이지의 URL/ID를 얻는다. (Business plan 전용인 `__notion-query-data-sources`를 쓰지 않는다.)
2. 정확히 1행이면 그 페이지 ID가 확정값이다.
3. 다수면 각 행의 `개발 서비스`(§2 서비스 포함)·`플랫폼` 속성으로 좁힌다. 그래도 모호하면 후보를 사용자에게 보여주고 고르게 한다.
...
```bash
ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
sh ${CLAUDE_PLUGIN_ROOT}/hooks/run-node.sh ${CLAUDE_PLUGIN_ROOT}/hooks/ensure-gitignore.js "$ROOT"
```
...
```
워크스페이스 설정 완료. 다음 단계:
  새 작업: /solution-create-task <작업번호>
  진행 중 작업 재개: /solution-choose-subtask
```
