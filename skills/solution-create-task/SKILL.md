...
선택된 taskType은 task.json에 영문 키(`feature`/`update`/`bugfix`)로 저장한다. Notion `작업 유형` select 값 변환(`신규 개발`/`변경/고도화`/`버그 수정`)은 publish-notion이 `WORKTYPE_LABEL`(정본 `hooks/lib/constants.json`) lookup으로 수행 — 본 skill이 한국어 라벨을 만들지 않는다. taskType은 라벨·세부작업 뷰 분기·update 수정 동작(state-schema §6)에 쓰일 뿐, 순차 단계 집합(pipeline)은 결정하지 않는다.
